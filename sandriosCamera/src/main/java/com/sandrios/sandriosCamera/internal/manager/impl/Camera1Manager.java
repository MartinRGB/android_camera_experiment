...
public class Camera1Manager extends BaseCameraManager<Integer, SurfaceHolder.Callback>
    ...
    public interface OnRectChangeListener
    {
        ...
        public void OnRectDetect(Boolean mBoolean);
    }
    private OnRectChangeListener mOnRectChangeListener = null;

    public void setOnRectChangeListener(OnRectChangeListener onRectChangeListener)
    {
        mOnRectChangeListener = onRectChangeListener;
...
//                    mFaceListener = new MyFaceDetectionListener();
//                    camera.setFaceDetectionListener(mFaceListener);
//                    camera.startFaceDetection();
                    ...
                    camera.setFaceDetectionListener(new Camera.FaceDetectionListener() {
                        ...
                        public void onFaceDetection(Camera.Face[] faces, Camera camera) {
                            if (faces.length == 0) {
                                mOnRectChangeListener.OnRectDetect(false);
                            } else if (faces.length > 0) {

                                mOnRectChangeListener.OnRectDetect(true);
                                int left = faces[0].rect.left;
                                int right = faces[0].rect.right;
                                int top = faces[0].rect.top;
                                int bottom = faces[0].rect.bottom;
                                mRect = new Rect(left, top, right, bottom);
                                mOnRectChangeListener.OnRectChange(mRect);
                            }
                        }
